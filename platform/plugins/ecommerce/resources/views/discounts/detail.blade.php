<div class="discount @if ($item->isExpired()) is-discount-disabled @endif">
    ...
    <div class="discount-inner">
        <p class="discount-code"> @if ($isCoupon) <span class="text-uppercase">{{ trans('plugins/ecommerce::discount.coupon_code') }}</span>: <b>{{ $item->code }}</b> @else <span class="text-uppercase">{{ trans('plugins/ecommerce::discount.discount_promotion') }}</span>: {{ $item->title }} @endif</p>
